...
class GitSpider(scrapy.Spider):
    ...
    def get_all_urls(self):
        with open('./urls_90000.txt', 'r') as urlfile:
            line = urlfile.readlines()
        return [s.strip() for s in line]

    def start_requests(self):
        # repo_list = self.fetch_git_repos('result.txt') #根据爬取下来的项目名全称构造出一个存放仓库名的list
        # diff_key_list = self.fetch_git_repos('keys.txt') #根据keys.txt文件构造一个10w条diff开头指令的list
        # for key in diff_key_list:
        #     #从数据库中读取对应的一条diff指令所对应的一个commitment，具体是啥得看数据库内容
        #     data = pd.read_sql("select * from commitment c where c.diff = '{}'".format(key), self.engine)
        #     for index, row in data.iterrows():
        #         #获得某次hashcode对应的数据库中的数据
        ...
        #                     break
        global allUrls
        allUrls = self.get_all_urls()
        #此处就相当于访问project对应commit版本的地址：https://github/com/{projectName}/commit/{hashCode}
        for url in allUrls:
    ...
    def parse(self, response):
        ...
        # parent hash
        #每个commit页面都会在一个地方放有其前一次commit的地址，此处就是获得这一个版本的信息
        ...
        changedFilesTag = response.css("div#toc ol.content li")
        for tag in changedFilesTag:
            a = tag.css("a::text").extract()
            b = ""
            for s in a:
                b += s
            c = b.strip() #得到的c字串就是github上面的文件名，盲猜是原本代码中的b_path

            a_fetch_url = "{}/{}/{}".format(target_repo, parent_hash, c)
            b_fetch_url = "{}/{}/{}".format(target_repo, cur_hash, c)
